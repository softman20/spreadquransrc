...
<?php if(!$bookmarked) { ?> Facebook.showBookmarkDialog(); <?php } ?>
...
function mouseaction(evt) {
 ...
 var oldLang= document.getElementById('hidTransLang').getValue();
...
//new Dialog().showMessage(mypos,eventFiredBy_ObjectId.substring(mypos));
   ...
   document.getElementById('selectLang_'+targetNbr).setValue(oldLang);
...
//hide others
	...
	Animation(targetId).to('height', 'auto').from('0px').to('width', '402px').from('0px').to('opacity', 1).from(0).show().go();
...
function changeReader(){
	...
	if(parseFloat(soura)<10) souraMp3="00"+soura;
	 ...
	 //new Dialog().showMessage(souraMp3);
	var souraName=selectList.getOptions()[selectList.getSelectedIndex()].getTitle();
...
//	new Dialog().showMessage('',soura);
	...
	ajax.post('http://prayers.jfntechnologie.com/tafsir_quran/souraAjax.php',queryParams);
	return false;
...
<fb:iframe id="ifr_header" width="728" height="230" name="ifr_header" frameborder="0" src="http://prayers.jfntechnologie.com/prayers_time/illa_salati_watch.php"></fb:iframe>
  ...
  <div id='eyadivMenu' class='popup' style='width:343px;direction:rtl'>
       ...
       <li><a href="http://apps.facebook.com/quran_tafsir/index.php">.:: <b>إرجع إلى الفهرس </b> ::.</a>
       <br /></li>
       <li>
        <span>
        <b>السورة</b>
...
?>
  ...
  <li>
            ...
            <li>
            <iframe src="http://www.facebook.com/plugins/like.php?href=http%3A%2F%2Fwww.facebook.com%2Ftafsir.quran&amp;layout=standard&amp;show_faces=false&amp;width=500&amp;action=like&amp;font=tahoma&amp;colorscheme=light&amp;height=35" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:290px; height:35px;" allowTransparency="true"></iframe>
            </li>                   
        </ul>
...
<img src='http://ga.webdigi.co.uk/fbga.php?googlecode=UA-4807913-9&googledomain=facebook.com&pagelink=list.php&pagetitle=list_page'></img>