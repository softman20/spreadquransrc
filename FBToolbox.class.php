...
class FbToolbox
{
	...
	public function __construct($apiKey, $secretKey)
	{
		...
		$this->facebook  = new Facebook($this->apiKey, $this->secretKey);
			...
			$this->fbuser = $this->facebook->require_login();
			...
			catch (Exception $ex) 
			{
				...
				$this->facebook->redirect($callbackUrl);
			}
...
$bookArray= $this->facebook->api_client->fql_query("select bookmarked from permissions where uid={$this->fbuser}");
if($bookArray[0]["bookmarked"]=="1") return true; else return false;
	...
	public function setUserTabFBML($user, $profile, $narrow)
	{
		try
		{
		$this->facebook->api_client->profile_setFBML('', $user, $profile, null, null, $narrow);
